...
7. You have an existing set of tooling and stuff that are really awful to abstract the hardware and you need a step up.
...
#### Level 3
self organizing app. Something kinda like Habitat. Apps are organized on a gossip network. Deployment is telling them which action to execute and letting them organize it themselves. They are self controlling, see autopilot pattern. The unit of deployment/machine is self registering or draining from the other resources (load balancer, database, etc) and talk with other machines in the same app to choreography its deployments based on the action asked. Unit of deployment is a self contained self organizing app with all needed sidecars.
